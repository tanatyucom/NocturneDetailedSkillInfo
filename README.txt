...
----------------------
Version 1.0.1 treats the incoming localized help string as authoritative,
including text produced by another help-description mod. It runs its Harmony
postfix at Priority.Last, uses a normalized copy only for analysis, and
appends details to the original string. If no supported detail is generated,
the original string is returned exactly.

Repeated calls remove only an exact terminal suffix previously generated by
this mod for the same skill during the current process. Generic Power: or
威力: text is never used as a truncation marker.

Validation
----------
See VALIDATION.md and docs\validation\ for the preserved Japanese/English
audit artifacts, reproducible procedure, PASS criteria, and regression tests.
